  ...
  app.post("/api/listings", async (req, res) => {
    ...
    if (process.env.GEMINI_API_KEY && images && images[0]) {
      try {
          ...
          contents: [
            {
              parts: [
                { text: `Act as a marketplace auditor. Analyze this waste material listing:
                  ...
                  Check for:
                  1. Accuracy: Does description match category?
                  2. Compliance: Is it prohibited (hazardous, illegal)?
                  3. Quality: Is the description sufficient?
                  
                  Return JSON: { "is_verified": boolean, "notes": string, "confidence": number }` 
                },
        ...
        if (result.is_verified && result.confidence > 0.8) {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [
          {
            parts: [
              { text: "Analyze this image of waste material and suggest a primary category from: Plastic, Metal, Paper, Organic, Electronic, Glass, Textile, Other. Return only the category name." },
